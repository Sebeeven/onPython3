(function ($, Backbone, _, app) {
    ...
    var FormView = TemplateView.extend({
        ...
        showErrors: function (errors) {
            _.map(errors, function (fieldErrors, name) {
                var field = $(':input[name=' + name + ']', this.form), label = $('label[for=' + field.attr('id') + ']', this.form);
                if(label.length === 0) {
    ...
    var HomepageView = TemplateView.extend({
        ...
        renderAddForm: function (event) {
            var view = new NewSprintView(), link = $(event.currentTarget);
            event.preventDefault();
            link.before(view.el);
            link.hide();
            view.render();
    ...
    var LoginView = FormView.extend({
        ...
        submit: function (event) {
            ...
            // };
            // $.post(app.apiLogin, data).done($.proxy(this.loginSuccess, this)).fail($.proxy(this.loginFailure, this));
        },
    ...
    var StatusView = TemplateView.extend({
        ...
        changeTask: function (task) {
            var changed = task.changedAttributes(), view = this.tasks[task.get('id')];
        ...
        renderAddForm: function (event) {
            var view = new AddTaskView(), link = $(event.currentTarget);
            event.preventDefault();
            link.before(view.el);
            link.hide();
            view.render();
        ...
        drop: function (event) {
            var dataTransfer = event.originalEvent.dataTransfer, task = dataTransfer.getData('application/model'), tasks, order;
    ...
    var TaskItemView = TemplateView.extend({
        ...
        drop: function (event) {
            var self = this, dataTransfer = event.originalEvent.dataTransfer, task = dataTransfer.getData('application/model'), tasks, order;
            if(event.stopPropagation) {
                event.stopPropagation();
            }
            task = app.tasks.get(task);
            ...
            if (task !== this.task) {
                ...
                tasks = app.tasks.filter(function (model) {
                    return model.get('id') !== task.get('id') && model.get('status') === self.task.get('status') && model.get('sprint') === self.task.get('sprint') && model.get('order') >= order;
                });
    ...
    var TaskDetailView = FormView.extend({
        ...
        editField: function (event) {
            var $this = $(event.currentTarget), value = $this.text().replace(/^\s+|\s+$/g, ''), field = $this.data('field');
        ...
        showErrors: function (errors) {
            _.map(errors, function (fieldErrors, name) {
                ...
                function appendError (msg) {
                    var parent = field.parent('.with-label'), error = this.errorTemplate({msg: msg});
                    if (parent.length === 0) {
                        field.before(error);
                    } else {
                        parent.before(error);
    ...
    var SprintView = TemplateView.extend({
        ...
        render: function () {
            ...
            _.each(this.tasks, function (task, taskId) {
        ...
        renderTask: function (task) {
            // var column = task.statusClass(), container = this.statuses[column], html = _.template('<div><%- task.get("name") %></div>', {task: task});
