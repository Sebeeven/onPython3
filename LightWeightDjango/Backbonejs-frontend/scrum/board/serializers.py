...
class TaskSerializer(serializers.ModelSerializer):
    assigned = serializers.SlugRelatedField(slug_field=User.USERNAME_FIELD, required=False, allow_null=True, queryset=User.objects.all())
        ...
        if self.instance and self.instance.pk:
            if value != self.instance.sprint:
                ...
                if value and value.end < date.today():
                    msg = _('Cannot assigne tasks to past sprints.')
                    raise serializers.ValidationError(msg)
            else:
                if value and value.end < date.today():
